...
public class Reserva {
    ...
    public void cadastrarReserva(Banco db) {
        String query = String.format(
                "INSERT INTO Reserva (data_reserva, metodo_pagamento, barbearia, cliente, nome_servico) VALUES ('%tF', '%s', '%s', '%s', '%s')",
                getDataReserva(), getMetodoPagamento(), getIdBarbearia(), getIdCliente(), getIdServico()); // Preenche
                                                                                                           // cada campo
                                                                                                           // da tabela
                                                                                                           // Reserva
                                                                                                           // atraves do
    ...
    public static boolean pesquisarReservaNoDia(Banco db, Date data, String barbeariaId) throws SQLException {
        String query = String.format("SELECT * FROM Reserva WHERE data_reserva = '%tF' and barbearia = '%s'", data,
                barbeariaId); // Realiza uma busca para a mesa na data informada
    ...
    public void editarReserva(Banco db) {
        String query = String.format(
                ...
                getIdServico(), getDataReserva(), getMetodoPagamento(), getIdBarbearia(), getIdCliente(), getId()); // Preenche
                                                                                                                    // cada
                                                                                                                    // campo
                                                                                                                    // da
                                                                                                                    // tabela
